...
RCT_EXPORT_METHOD(account_BindNFTAccount:(NSString*)account password:(NSString*)password  successCallback:(RCTResponseSenderBlock)successCallback) {
    RCTLogInfo(@"account_BindNFTAccount");
    long sdkStatus = [dcapi account_BindNFTAccount:account password:password];
...
// 99:其他异常
// 4:还没有建立到存储节点的连接 5:加密数据过程出错 6:区块链相关错误 7:签名错误 8:用户有效期已过
RCT_EXPORT_METHOD(account_BindNFTAccountWithAppBcAccount:(NSString*)account password:(NSString*)password  successCallback:(RCTResponseSenderBlock)successCallback) {
    RCTLogInfo(@"account_BindNFTAccountWithAppBcAccount");
    long sdkStatus = [dcapi account_BindNFTAccountWithAppBcAccount:account password:password];
    successCallback(@[@(sdkStatus)]);
}
...
RCT_EXPORT_METHOD(account_NFTAccountLogin:(NSString*)account password:(NSString*)password  successCallback:(RCTResponseSenderBlock)successCallback errorCallback:(RCTResponseSenderBlock)errorCallback) {
    ...
    long login = [dcapi account_NFTAccountLogin:account password:password];
    if(login == 0){
...
//NFT账号密码修改
RCT_EXPORT_METHOD(account_NFTAccountPasswordModify:(NSString*)account password:(NSString*)password  successCallback:(RCTResponseSenderBlock)successCallback) {
    RCTLogInfo(@"account_NFTAccountPasswordModify");
    long sdkStatus = [dcapi account_NFTAccountPasswordModify:account password:password];
    successCallback(@[@(sdkStatus)]);
}

//子账号NFT账号密码修改
RCT_EXPORT_METHOD(account_AppNFTAccountPasswordModify:(NSString*)account password:(NSString*)password  successCallback:(RCTResponseSenderBlock)successCallback) {
    RCTLogInfo(@"account_AppNFTAccountPasswordModify");
    long sdkStatus = [dcapi account_AppNFTAccountPasswordModify:account password:password];
    successCallback(@[@(sdkStatus)]);
}
