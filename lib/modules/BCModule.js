...
export const DCpeerState = (multiaddr) => {
  return new Promise((resolve, reject) => {
    const successCallback = (state) => {
      const peerStadus = state ? JSON.parse(state) : '';
      resolve({state: peerStadus && peerStadus.Status ? peerStadus.Status : ''});
    };
...
// 购买存储
export const DCsubscribeStorage = (packageId) => {
  return new Promise(resolve => {
    ...
    BCModule.bc_SubscribeStorage(packageId, successCallback, errorCallback);
  });
};

// 为他人购买存储
export const BCsubscribeStorageForNFTAccount = (packageId, nftAccount) => {
  return new Promise(resolve => {
    const successCallback = () => {
      //console.log('bc_SubscribeStorage success');
      resolve({});
    ...
    BCModule.bc_SubscribeStorageForNFTAccount(packageId, nftAccount, successCallback, errorCallback);
  });
};
