...
// 修改数据表
export const DCrebuildIndex = (threadid, collectionName, field, uniqueFlag) => {
  return new Promise(resolve => {
    const successCallback = () => {
      //console.log('rebuildIndex success');
      resolve({});
    };
    const errorCallback = (error) => {
      //console.log('rebuildIndex error', error);
      resolve({error});
    };
    DBModule.rebuildIndex(
      threadid,
      collectionName, 
      field, 
      uniqueFlag,
...
// 获取数据库操作记录数
export const DBGetDBRecordsCount = (threadid) => {
  return new Promise(resolve => {
    const successCallback = (count) => {
      //console.log('rebuildIndex success');
      resolve({count});
    };
    DBModule.db_GetDBRecordsCount(
      threadid,
      successCallback,
    );
  });
...
// 导出数据库到文件
export const DBExportDBToFile = (threadId, path, rk) => {
  return new Promise(resolve => {
...
export const DBPreloadDBFromDC = (threadId, fid, dbName, rk, sk, jsonCollections) => {
  return new Promise(resolve => {
    const successCallback = (jsonCollections) => {
      //console.log('rebuildIndex success');
