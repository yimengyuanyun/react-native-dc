...
// 给用户充值余额
export const DCaddBalanceForTest = (balance) => {
  return new Promise(resolve => {
    const successCallback = (bool) => {
...
// account转address
export const DCgetSS58AddressForAccount = (account) => {
  return new Promise((resolve) => {
    const successCallback = (address) => {
      console.log("dc_GetSS58AddressForAccount success", address);
      resolve(address);
    };
    const errorCallback = (error) => {
      console.log("dc_GetSS58AddressForAccount error", error);
      resolve({ error });
    };
    DCModule.dc_GetSS58AddressForAccount(account, successCallback, errorCallback);
  });
};


// 启动p2p通信服务
export const dc_StartP2pServer = (receiver, model) => {
  return new Promise((resolve) => {
    const successCallback = (bool) => {
